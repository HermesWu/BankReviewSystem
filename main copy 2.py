...
def batch_analyze_reviews(reviews, batch_size=10):
    """
    将评价打包分批发给大模型，极大提升速度并防止 API 超时
    """
    analyzed_data = []
    
    # 按照 batch_size (默认 10 条) 将全部评价分块
    for i in range(0, len(reviews), batch_size):
        ...
        {{
            "results": [
                {{"情感倾向": "负向", "问题分类": "闪退", "一句话总结": "启动频繁闪退"}},
                {{"情感倾向": "正向", "问题分类": "其他", "一句话总结": "转账速度很快"}}
            ]
        }}
        """

        try:
            # 3. 统一调用 API（使用 json_object 模式保证返回合法 JSON）
            ...
            # 4. 终极防崩溃解析器
            # 使用 chr(96) 动态生成反引号，完美避开平台前端面板的渲染截断 Bug
            backticks = chr(96) * 3
            ...
            # 兜底机制：即使这批数据解析全线崩溃，也不影响整个程序，填入默认值继续跑下一批
            for review in batch:
                review["情感倾向"] = "解析失败"
                review["问题分类"] = "解析失败"
                review["一句话总结"] = "解析失败"
        ...
        # 保护机制：每批处理完稍微休息 1 秒，防止触发阿里云 API 的并发限制
        time.sleep(1)
