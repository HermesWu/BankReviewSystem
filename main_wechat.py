...
def generate_report(results, query_days=7, top_n=5):
   ...
   #     f.write(html_content)
    ...
    return file_path  # 🌟 记得返回这个新路径，方便后面发企微

# ======= 优化 2：历史数据累积模块 =======
# ======= 优化 2 升级版：智能去重的数据沉淀模块 =======
def save_to_history(new_data, history_file="app_reviews_history.csv"):
    """将新数据与历史数据合并，并自动去重"""
    if not new_data:
        return
        
    # 1. 把本次抓取的新数据转换为 DataFrame
    df_new = pd.DataFrame(new_data)
    cols = ["来源", "日期", "具体时间", "标题", "内容", "评分", "情感倾向", "问题分类", "一句话总结"]
    # 只保留存在的列
    ...
    if os.path.exists(history_file):
        try:
            ...
            print(f"⚠️ 读取历史文件失败，将创建新文件: {e}")
            df_combined = df_new
    else:
        df_combined = df_new

    # 3. 🌟 核心魔法：全量去重！
    # 只要“来源”、“具体时间”和“内容”这三个字段完全一样，就认为是同一条评论，只保留最后抓取的那条
    df_combined.drop_duplicates(subset=['来源', '具体时间', '内容'], keep='last', inplace=True)
    
    # 按时间降序排列一下，让最新的评论在最上面
    df_combined.sort_values(by='具体时间', ascending=False, inplace=True)

    # 4. 重新覆写回 CSV 文件
    try:
        df_combined.to_csv(history_file, index=False, encoding='utf-8-sig')
        print(f"💾 数据已成功同步并去重！当前总数据量库: {len(df_combined)} 条")
    except Exception as e:
        print(f"❌ 历史数据存档失败: {e}")

# ======= 升级版：企业微信图文+附件推送模块 (稳妥版) =======
# ======= 优化 1：智能预警推送模块 =======
def send_wecom_message(summary_text, results, html_file_path="全渠道评价分析报告.html"):
    ROBOT_KEY = "44ba0a2e-4510-4804-9d3c-f2c53f39e267" 
    webhook_url = f"https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key={ROBOT_KEY}"
    
    # 🎯 智能逻辑 A：统计严重问题，准备 @ 负责人
    bad_reviews = [x for x in results if x.get('情感倾向') == '负向']
    
    # 设定“值班表”（这里填入同事在企微里的 UserID，不是昵称哦）
    # 如果不知道 UserID，可以先用文字代替，或者在企微后台查看
    dev_leads = ["@吴开发"]  # 假设这是开发负责人
    pd_leads = ["@张产品"]   # 假设这是产品负责人
    
    alert_at = []
    for rev in bad_reviews:
        content = rev.get('内容', '')
        if any(word in content for word in ["闪退", "死机", "黑屏", "打不开"]):
            if dev_leads[0] not in alert_at: alert_at.append(dev_leads[0])
        if any(word in content for word in ["流程", "开户", "难用", "找不到"]):
            if pd_leads[0] not in alert_at: alert_at.append(pd_leads[0])

    at_text = f"\n\n请相关负责人关注：{' '.join(alert_at)}" if alert_at else ""

    # 🎯 智能逻辑 B：如果没有差评，可以选择不推送附件，只发个简单通知
    if len(bad_reviews) == 0:
        content_text = f"✅ **今日监控报告**\n\n项目：中信银行APP\n状态：未发现新增差评。大家辛苦了，继续保持！"
        # 这里直接 return 掉，不发附件了，省去大家点开看好评的时间
        # requests.post(webhook_url, json={"msgtype": "markdown", "markdown": {"content": content_text}})
        # return 
    else:
        content_text = f"🚨 **APP评价差评预警**\n\n<font color=\"warning\">{summary_text}</font>{at_text}\n\n> 💡 详细分析报告已作为附件发送，请查收 👇"
    ...
    if os.path.exists(html_file_path):
        upload_url = f"https://qyapi.weixin.qq.com/cgi-bin/webhook/upload_media?key={ROBOT_KEY}&type=file"
        try:
            with open(html_file_path, 'rb') as f:
                files = {'media': (os.path.basename(html_file_path), f, 'text/html')}
                upload_res = requests.post(upload_url, files=files).json()
            if upload_res.get('errcode') == 0:
                requests.post(webhook_url, json={"msgtype": "file", "file": {"media_id": upload_res['media_id']}})
        except: pass
...
def run_agent_task(query_days=7):
    ...
    # 🌟 拿到带路径的报告文件名
    report_path = generate_report(final_results, query_days=query_days, top_n=TOP_N)

    # 2. 呼叫管家，把数据存起来
    save_to_db(final_results)

    # 🌟 优化 2：运行完立刻存入历史档案
    # save_to_history(final_results)
    ...
    # 👈 整个程序只在这里调用一次推送！
    # 🌟 优化 1：调用升级后的智能推送（传入了完整的 final_results）
    # 🌟 这里的参数传入刚才生成的完整路径 report_path
    send_wecom_message(summary, final_results, html_file_path=report_path)
