...
def send_wecom_message(summary_text, html_file_path="全渠道评价分析报告.html"):
    
    # ⚠️ 请把你的 Webhook 链接中，key= 后面的那串英文数字单独复制到这里！
    ...
    # 🌟 第一步：发送 Markdown 格式的统计摘要
    text_data = {
        "msgtype": "markdown",
        "markdown": {
            "content": f"📢 **APP全渠道评价每日监控**\n\n<font color=\"info\">{summary_text}</font>\n\n> 💡 完整的数据可视化大屏（HTML文件）已作为附件发送，请点击下方文件直接预览 👇"
        }
    }
    try:
        requests.post(webhook_url, headers=headers, data=json.dumps(text_data))
    except Exception as e:
        print(f"❌ 推送文本失败: {e}")

    # 🌟 第二步：把本地的 HTML 报告作为文件附件上传到企微群
    if os.path.exists(html_file_path):
        ...
        try:
            ...
            if upload_res.get('errcode') == 0:
                ...
                requests.post(webhook_url, headers=headers, data=json.dumps(file_data))
                print("✅ 报告附件已成功发送到群里！")
            else:
